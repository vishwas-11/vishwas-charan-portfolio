...
export const AnimatedThemeToggle = ({ className }: { className?: string }) => {
  ...
  if (!mounted) {
    return (
      <Button
        className={cn("px-2.5 h-10 w-10 rounded-full border-0 bg-muted hover:bg-muted/75 text-foreground transition-all duration-300 hover:-translate-y-1 hover:shadow-lg", className)}
        variant="outline"
      >
  ...
  // Use the same startViewTransition logic as before so it seamlessly integrates with the rest of the app's smooth transitions
  ...
  return (
    <Button
      ...
      className={cn("px-2.5 h-10 w-10 rounded-full border-0 bg-muted hover:bg-muted/75 text-foreground transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-lg", className)}
      variant="outline"
    >
...
const SolarSwitch = ({ isDark }: { isDark: boolean }) => {
  ...
  return (
    <motion.div animate={isDark ? "checked" : "unchecked"} className="flex items-center justify-center transition-transform duration-500 ease-out group-hover:rotate-12 group-hover:scale-110">
      <motion.svg
        width="20"
        height="20"
        viewBox="0 0 25 25"
